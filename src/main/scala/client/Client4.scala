...
object Client4 extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    val playerID1 = clientPrivateResource.use { client =>
      for {
        _ <- client.send(WSFrame.Text(s"registration $name 4000"))
    ...
    val playerTableID =
      clientSharedResource.use { client =>
        for {
          playerID <- playerID1
          _ <- client.send(WSFrame.Text(s"game $playerID 10 400"))
